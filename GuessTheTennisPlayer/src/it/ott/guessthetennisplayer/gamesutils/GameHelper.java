 ...
 */
package it.ott.guessthetennisplayer.gamesutils;
...
public class GameHelper implements GooglePlayServicesClient.ConnectionCallbacks, GooglePlayServicesClient.OnConnectionFailedListener {
	...
	public interface GameHelperListener {
		 ...
		 * 
		 * @link{GamesHelper#beginUserInitiatedSignIn . Note that not all calls to this method mean an error; it may be a result of the fact
		 *                                            that automatic sign-in could not proceed because user interaction was required (consent
		 *                                            dialogs). So implementations of this method should NOT display an error message unless a
	...
	/**
	 * The Activity we are bound to. We need to keep a reference to the Activity because some games methods require an Activity (a Context won't
	 * do). We are careful not to leak these references: we release them on onStop().
	 */
	...
	/*
	 * Whether user has specifically requested that the sign-in process begin. If mUserInitiatedSignIn is false, we're in the automatic sign-in
	 * attempt that we try once the Activity is started -- if true, then the user has already clicked a "Sign-In" button or something similar
	 */
	...
	/*
	 * If we got an invitation id when we connected to the games client, it's here. Otherwise, it's null.
	 */
	...
	/**
	 * Construct a GameHelper object, initially tied to the given Activity. After constructing this object, call @link{setup} from the
	...
	/**
	 * Performs setup on this GameHelper object. Call this from the onCreate() method of your Activity. This will create the clients and do a
	 * few other initialization tasks. Next, call @link{#onStart} from the onStart() method of your Activity.
	 * 
	 * @param listener
	 *            The listener to be notified of sign-in events.
	 * @param clientsToUse
	 *            The clients to use. Use a combination of CLIENT_GAMES, CLIENT_PLUS and CLIENT_APPSTATE, or CLIENT_ALL to request all clients.
	 * @param additionalScopes
	 *            Any scopes to be used that are outside of the ones defined in the Scopes class. I.E. for YouTube uploads one would add
	...
	/**
	 * Returns the GamesClient object. In order to call this method, you must have called @link{setup} with a set of clients that includes
	 * CLIENT_GAMES.
	 */
	...
	/**
	 * Returns the AppStateClient object. In order to call this method, you must have called @link{#setup} with a set of clients that includes
	 * CLIENT_APPSTATE.
	 */
	...
	/**
	 * Returns the PlusClient object. In order to call this method, you must have called @link{#setup} with a set of clients that includes
	 * CLIENT_PLUS.
	 */
	...
	/**
	 * Handle activity result. Call this method from your Activity's onActivityResult callback. If the activity result pertains to the sign-in
	...
	/**
	 * Starts a user-initiated sign-in flow. This should be called when the user clicks on a "Sign In" button. As a result,
	 * authentication/consent dialogs may show up. At the end of the process, the GameHelperListener's onSignInSucceeded() or onSignInFailed()
	...
	void startConnections() {
		if (!checkState(TYPE_GAMEHELPER_BUG, "startConnections", "startConnections should " + "only get called when disconnected.",
				STATE_DISCONNECTED)) {
			return;
		}
	...
	void connectCurrentClient() {
		...
		if (!checkState(TYPE_GAMEHELPER_BUG, "connectCurrentClient", "connectCurrentClient " + "should only get called when connecting.",
				STATE_CONNECTING)) {
			return;
		}
	...
	public void reconnectClients(int whatClients) {
		checkState(TYPE_DEVELOPER_ERROR, "reconnectClients", "reconnectClients should " + "only be called when connected. Proceeding anyway.",
				STATE_CONNECTED);
	...
	void succeedSignIn() {
		checkState(TYPE_GAMEHELPER_BUG, "succeedSignIn", "succeedSignIn should only "
				+ "get called in the connecting or connected state. Proceeding anyway.", STATE_CONNECTING, STATE_CONNECTED);
	...
	/**
	 * Attempts to resolve a connection failure. This will usually involve starting a UI flow that lets the user give the appropriate consents
	...
	/**
	 * Give up on signing in due to an error. Shows the appropriate error message to the user, using a standard error dialog as appropriate to
	 * the cause of the error. That dialog will indicate to the user how the problem can be solved (for example, re-enable Google Play Services,
